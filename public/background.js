...
// This function returns the appropriate job desciption className based on the url.
function getJobDescriptionClassName(url) {
  // If the URL starts with the LinkedIn list view URL, return the list view className, else return the detail view className.
    return url.startsWith(linkedInListViewURL) ?
        "jobs-search__job-details--container" : "jobs-description-content__text"
}

// This function grabs the hob description text from the web page.
  ...
  // Check if the tab is fully loaded and active.
    if (changeInfo.status === "complete" && tab.active) {
      // Check if the URL of the tab matches the LinkedIn list or detail view URL
        if (
            tab.url?.startsWith(linkedInListViewURL) || tab.url?.startsWith(linkedInDetailView)
        ) {
          // Execute the grabJobDescription function on the current tab and store the result in the local storage.
